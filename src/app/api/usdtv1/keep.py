...
@router.get("/active/predictions", response_model=PaginatedListResponse[OpponentRead])
async def get_all_latest_opponents(
    request: Request,
    db: Annotated[AsyncSession, Depends(async_get_db)],
    page: int,
    items_per_page: int
    ...
    """
    - Returns a paginated list of Opponents
    """
    users_data = await crud_predictions.get_multi_joined(
        db=db,
        offset=compute_offset(page, items_per_page),
